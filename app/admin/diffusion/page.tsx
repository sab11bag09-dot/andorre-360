...
const PUBLICATION_GRID_TEMPLATE = "minmax(0, 1fr) 180px 170px 130px";
...
export default async function DiffusionPage() {
  ...
  const usedZonesCount = new Set(
    activePublications.map((publication) => publication.zone),
  ).size;
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Button href="/admin/diffusion/simulation" variant="outline">
              Simuler l’accueil
            </Button>

            <Button href="/admin/articles/nouveau">Nouveau contenu</Button>
          </>
        }
      />

      <section className="grid gap-5 py-8 md:grid-cols-3">
        <StatCard title="Missions actives" value={activePublications.length} />

        <StatCard title="Zones utilisées" value={usedZonesCount} />
      ...
      <section>
        {activePublications.length === 0 ? (
          ...
          <DataTable
            ...
            gridTemplateColumns={PUBLICATION_GRID_TEMPLATE}
          >
            {activePublications.map((publication) => (
              <DataTableRow key={publication.id} className="items-start py-6">
                <div className="min-w-0">
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    ...
                    <Button href={`/admin/articles/${publication.article.id}`}>
                      Éditer
                    </Button>
                  </div>

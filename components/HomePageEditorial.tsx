...
export default async function HomePageEditorial() {
  const [editorialLayout, publishedArticles, activeAutomatedPublicationCount] =
    await Promise.all([
  ...
  const hasActiveAutomatedComposition = activeAutomatedPublicationCount > 0;
   ...
   */
  const hero = editorialLayout.hero ?? availableArticles.shift() ?? null;

  const feature = editorialLayout.feature ?? availableArticles.shift() ?? null;
  ...
  const grandFormat =
    ...
    (!hasActiveAutomatedComposition
      ? (availableArticles.shift() ?? null)
      : null);
  ...
  if (!hero) {
    return (
      <main className="min-h-screen bg-black p-8 text-white">
        <h1 className="font-serif text-3xl">Aucune Une principale définie</h1>

        <p className="mt-4 text-gray-400">
          Crée une mission éditoriale avec la zone « Une principale ».
        </p>
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-black text-white">
      ...
      <section className="mx-auto max-w-7xl px-6 py-10 md:px-10 md:py-14">
        <div className="grid items-start gap-10 lg:grid-cols-6">
          {/* PARTIE GAUCHE */}
          ...
          <div className="flex flex-col space-y-10 lg:col-span-4">
            ...
            <section className="h-[520px] w-full px-0 pb-0">
          ...
          <aside className="space-y-9 lg:col-span-2 lg:flex lg:flex-col lg:gap-9 lg:space-y-0 lg:border-l lg:border-gray-800 lg:pl-8">
            ...
            <section className="h-[620px] overflow-hidden border-b border-gray-800 pb-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h2 className="mt-1 font-serif text-3xl">L’Édito</h2>
                </div>
              </div>
              ...
              {editorial ? (
                ...
                >
                  ...
                  <h3 className="mt-3 line-clamp-2 font-serif text-3xl leading-snug transition group-hover:text-yellow-500">
                    {editorial.title}
                  </h3>
            ...
            <section className="h-[760px] overflow-hidden border-t-2 border-yellow-500 pt-5">
              ...
              <div className="divide-y divide-gray-800">
                {cards.slice(1, 5).map((article, index) => (
                  <Link
                    key={article.id}
                    href={`/article/${article.slug}`}
            ...
            {discover.length > 0 && (
              <section className="mt-12 h-[520px] overflow-hidden rounded-xl bg-zinc-900 p-6 lg:mt-0 lg:flex lg:flex-1 lg:flex-col">
                ...
                <h2 className="mt-2 font-serif text-2xl">À découvrir</h2>

                <div className="mt-5 min-h-0 overflow-hidden divide-y divide-zinc-700">
                  {discover.slice(0, 3).map((article) => (
                    <Link
                      key={article.id}
                      href={`/article/${article.slug}`}
                    ...
                    >
                      <div>
                        ...
                        <h3 className="mt-1 line-clamp-2 font-serif leading-snug transition group-hover:text-yellow-500">
                          {article.title}
                        </h3>
                      ...
                      <span className="mt-4 shrink-0 text-yellow-500">→</span>
                    </Link>
                  ))}
                </div>
              </section>
